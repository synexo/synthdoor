...
const STATIONS = [
  ...
  { dist: 600, name: 'Europa Colony', mult: 0 } // Destination
];
...
class EuropaTrail extends GameBase {
  ...
  async _showInstructions() {
    ...
    const sections = [
      { hdr: 'THE VOYAGE', col: C.BRIGHT_YELLOW, text: [
        'You are the commander of a colony ship bound for Europa.',
        'The journey covers 600 Million Kilometers (Mkm) of deep space.',
        'You must manage your crew, ship systems, and resources to survive.'
      ]},
      { hdr: 'RESOURCES', col: C.BRIGHT_CYAN, text: [
        'CREDITS : The currency of the solar system.',
        'FUEL    : He-3 Isotopes. Required to move the ship.',
        'RATIONS : Food and water. Prevents crew starvation and disease.',
        'PARTS   : Used to repair the Hull, Reactor, and Life Support.'
      ]},
      { hdr: 'SHIP SYSTEMS', col: C.BRIGHT_GREEN, text: [
        ...
        'Keep them repaired using PARTS.'
      ]},
      { hdr: 'STATIONS & PACING', col: C.BRIGHT_YELLOW, text: [
        'You can stop at planetary outposts to buy supplies, but prices rise',
        'the further you get from Earth.',
        'You can change your flight pace to travel faster, but it burns',
        'exponentially more fuel and increases the risk of ship damage.'
      ]}
    ];
  ...
  async _playGame() {
    ...
    // Main loop
    while (this.s.distance < 600 && !this.s.dead) {
      await this._processTurn();
      if (this.s.dead) break;

      // Check arrival
      if (this.s.distance >= 600) {
        await this._doVictory();
        const score = this.s.credits + (this._countAlive() * 1000) + this.s.hull * 10;
  ...
  async _setupPhase() {
    ...
    const profs = [
      { id: 1, name: 'Corporate Banker',  desc: 'Starts with 8000 Credits.' },
      { id: 2, name: 'Chief Engineer',    desc: 'Starts with 4000 Credits, bonus repair skills.' },
      { id: 3, name: 'Veteran Pilot',     desc: 'Starts with 4000 Credits, uses 10% less fuel.' }
    ];
    ...
    this.s.credits = choice === 1 ? 8000 : 4000;
  ...
  async _advanceTime() {
    ...
    // Calculate costs
    let fuelBurn = [0, 8, 20, 45][this.s.pace];
    if (this.s.prof === 3) fuelBurn = Math.floor(fuelBurn * 0.9); // Pilot bonus

    let distGain = [0, 15, 35, 60][this.s.pace];
    distGain += Math.floor(Math.random() * 6) - 3; // variance
    
    const aliveCount = this._countAlive();
    let rationBurn = [0, 1, 3, 5][this.s.rationLv] * aliveCount;
    ...
    if (this.s.pace === 3) {
      this.s.reactor -= Utils.randInt(2, 6);
      this.s.hull -= Utils.randInt(0, 3);
    } else {
      this.s.reactor -= Utils.randInt(0, 2);
    }
    this.s.lifeSupport -= Utils.randInt(0, 2);

    // Life support check
    if (this.s.lifeSupport <= 20) {
  ...
  async _repairShip() {
    ...
    t.println('  Which system to repair? (1 part = +10% to system)');
    t.println();
    ...
    const eff = (this.s.prof === 2) ? 15 : 10; // Engineer gets 15% per part
    const boost = amt * eff;
  ...
  async _miningMinigame() {
    ...
    this.s.fuel -= 2;
    this.s.week++; // Takes time

    this._sectionHeader('TACTICAL ASTEROID MINING', C.BRIGHT_YELLOW);
    t.println();
    t.setColor(C.WHITE, C.BLACK);
    t.println('  Targeting rogue asteroid. Laser banks armed.');
    ...
    t.println();
    if (typed !== word) {
      this._notice('TARGETING FAILURE! DEBRIS IMPACT!', C.BRIGHT_RED);
      this.s.hull -= Utils.randInt(5, 15);
    } else {
      if (elapsed <= 2.0) {
        this._notice('PERFECT STRIKE! MAXIMUM YIELD ACQUIRED.', C.BRIGHT_GREEN);
        this.s.rations += Utils.randInt(20, 40);
        this.s.fuel += Utils.randInt(10, 20);
        this.s.parts += Utils.randInt(2, 5);
      } else if (elapsed <= 4.0) {
        this._notice('TARGET DESTROYED. MODERATE YIELD.', C.GREEN);
        this.s.rations += Utils.randInt(10, 20);
        this.s.fuel += Utils.randInt(5, 12);
        this.s.parts += Utils.randInt(0, 2);
      } else {
        this._notice('TARGET DESTROYED, BUT TOO SLOW. MINIMAL YIELD.', C.YELLOW);
        this.s.rations += Utils.randInt(1, 10);
        this.s.fuel += Utils.randInt(1, 5);
      }
    }
  ...
  async _doStation(station, isInit = false) {
    ...
    const prices = {
      fuel:    Math.floor(10 * station.mult),
      rations: Math.floor(5 * station.mult),
      parts:   Math.floor(150 * station.mult)
    };
    ...
    while (true) {
      ...
      t.setColor(C.BRIGHT_YELLOW, C.BLACK); t.println('  ITEMS FOR SALE:');
      t.setColor(C.WHITE, C.BLACK);
      t.println(`   [1] Fuel Unit     ($${prices.fuel})`);
      t.println(`   [2] Ration Pack   ($${prices.rations})`);
      t.println(`   [3] Spare Part    ($${prices.parts})`);
      t.println(`   [4] Doctor Visit  ($${Math.floor(200 * station.mult)})`);
      t.println(`   [5] Return to Ship`);
      t.println();

      t.setColor(C.BRIGHT_CYAN, C.BLACK); t.write('  Choice (1-5): '); t.resetAttrs();
      const line = await t.readLine({maxLen:1});
      ...
      if (choice >= 1 && choice <= 3) {
        ...
        if (total > this.s.credits) {
          ...
          this._notice(`Purchased ${qty} ${item}s.`, C.BRIGHT_GREEN);
          await t.waitKey();
        }
      }

      if (choice === 4) {
        const cost = Math.floor(200 * station.mult);
        if (this.s.credits < cost) {
  ...
  async _randomEvent() {
    const t = this.terminal;
    // Base 30% chance of an event if moving
    if (Utils.chance(0.70)) return;
    ...
    if (roll <= 15) {
      ...
      this.s.reactor -= Utils.randInt(5, 15);
      ...
      const loss = Utils.randInt(10, 30);
      this.s.rations -= loss;
      if (this.s.rations < 0) this.s.rations = 0;
      ...
      if (ok) {
        if (Utils.chance(0.7)) {
          const f = Utils.randInt(10, 30);
          const p = Utils.randInt(1, 4);
          this.s.fuel += f;
          this.s.parts += p;
          ...
          this.s.hull -= Utils.randInt(10, 25);
        }
      }
    } else {
      ...
      if (ok) {
        if (Utils.chance(0.5)) {
          ...
          this.s.fuel -= Utils.randInt(10, 30);
          if (this.s.fuel < 0) this.s.fuel = 0;
        } else {
          this._notice('Failed to outrun them! Hull breached by weapons fire!', C.BRIGHT_RED);
          this.s.hull -= Utils.randInt(15, 30);
          this.s.rations -= Utils.randInt(10, 40);
          if (this.s.rations < 0) this.s.rations = 0;
        }
      } else {
        await this._miningMinigame(); // Re-use minigame for tactical combat
      }
    }
  }
  ...
  _dateDistanceHeader() {
    const t = this.terminal;
    const pct = Math.floor(Math.min(100, (this.s.distance / 600) * 100));
