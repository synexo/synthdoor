...
export class App {
  ...
  /**
   * Return true if (col, row) is clickable as a "single-key" target.
   *
   * Two matching patterns:
   *
   *   1. Alphanumeric whose horizontal neighbours are NOT alphanumeric.
   *      Catches the conventional BBS menu-key style:
   *          [L]ogin   (A)bort   1. New game   Q.uit
   *      Note: neighbours need only be non-alphanumeric — punctuation,
   *      brackets, whitespace, box-drawing chars all qualify.
   *
   *   2. Non-alphanumeric character explicitly wrapped in literal
   *      square brackets:
   *          [%]   [!]   [?]   [*]   [/]
   *      The left neighbour must be '[' and the right neighbour must
   *      be ']'. This is intentionally narrow: only bracketed punctuation
   *      counts, so flowing prose ("the % symbol") stays non-clickable.
   *
   * The historical name '_isLoneAlphaNum' is retained to minimise diff
   * surface; the predicate now answers the broader question "is this a
   * single-key click target?".
   */
  _isLoneAlphaNum(col, row) {
    ...
    // Pattern 1: alphanumeric with non-alphanumeric neighbours.
    if (this._isAlphaNum(ch)) {
      return !this._isAlphaNum(left) && !this._isAlphaNum(right);
    }

    // Pattern 2: non-alphanumeric wrapped in literal square brackets.
    // The brackets themselves are not click targets (their neighbours
    // wouldn't be '[' and ']'), which is correct.
    return left === '[' && right === ']';
  }
