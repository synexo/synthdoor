...
class MenuLoader {
   ...
   * @returns {MenuDef}
   */
  loadTop(discoveredGames) {
    const topPath = path.join(this.menusDir, 'top.yaml');

    if (!fs.existsSync(topPath)) {
      this._autoGenerateTop(topPath, discoveredGames);
    }

    return this.load('top');
  }
  ...
  _autoGenerateTop(topPath, games) {
    ...
    console.log('');
    console.log('╔══════════════════════════════════════════════════════════╗');
    console.log('║  MENU SYSTEM: top.yaml not found — auto-generated.       ║');
    console.log(`║  Created: ${topPath.padEnd(46)} ║`);
    console.log('║  Edit this file to customise your top-level menu.        ║');
    console.log('║  See docs/MENU.md for the full schema reference.         ║');
    console.log('╚══════════════════════════════════════════════════════════╝');
    console.log('');
  }
  ...
  _normaliseMenuDef(raw, filePath, depth) {
    ...
    if (def.ansifile && def.underlay) {
      console.warn(`[MenuLoader] ${filePath}: both ansifile and underlay set; ansifile takes precedence.`);
      def.underlay = null;
    }
    ...
    if (!rawSel && !def.ansifile) {
      ...
      console.warn(`[MenuLoader] ${filePath}: no selections defined.`);
    }
  ...
  _normaliseSelection(raw, filePath, depth, idx) {
    ...
    if (type === 'action') {
      ...
      if (!BUILTIN_ACTIONS.has(sel.target)) {
        console.warn(`[MenuLoader] ${loc}: unknown action "${sel.target}" — will be ignored at runtime`);
      }
    }
    ...
    for (const [key, val] of Object.entries(raw)) {
      ...
      if (name in COLOR_NAMES) {
        ...
        if (!isNaN(n) && n >= 0 && n <= 15) {
          ...
          console.warn(`[MenuLoader] ${filePath}: unknown color value "${val}" for "${key}" — ignored`);
        }
      }
    }
